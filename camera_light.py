...
def load_config():
    ...
    defaults = {
        ...
        "off_delay_seconds": 3,
        "on_settings": {"brightness": 40, "temperature": 162},
...
def is_camera_active():
    ...
    # Fallback: IORegistry for USB cameras (Insta360)
    try:
        # Check for Insta360 specifically in IOUSB
        result = subprocess.run(
            ['ioreg', '-p', 'IOUSB', '-w0'],
            capture_output=True, text=True, timeout=5
        )
        # Note: This checks presence, monitoring "active" status via registry for USB is harder,
        # but the log stream is the primary "active" detector. 
        # However, for polling "active" usage, we might need to rely on the side-effect 
        # that the camera is attached. 
        # Actually, standard `ioreg` doesn't easily show "Streaming" for USB UVC.
        # effectively `camera_check` (Swift) allows checking active state.
        # If camera_check fails, we might just return False here rather than True on just presence.
        pass 
    except Exception:
        pass
...
class CameraMonitor:
    ...
    def start_log_stream(self):
        ...
        cmd = [
            'log', 'stream',
            '--predicate',
            'subsystem == "com.apple.cameracapture" AND eventMessage CONTAINS "Input"',
            '--style', 'ndjson'
        ]
    ...
    def _process_log_line(self, line):
        """Process log line for camera events."""
        try:
            # We are using ndjson style or just parsing text
            # For robustness with the previous 'text' decoding in Popen:
            
            # Logic found: "addInput:" means START, "removeInput:" means STOP
            if "Insta360" in line:
                is_start = "addInput:" in line
                is_stop = "removeInput:" in line
                ...
                if is_start:
                    with self.lock:
                        if not self.light_is_on:
                            ...
                            print(f"Camera START (log stream) - {datetime.now()}", flush=True)
                            self.callback(True)
                     ...
                     with self.lock:
                        if self.light_is_on:
                            self.light_is_on = False
                            print(f"Camera STOP (log stream) - {datetime.now()}", flush=True)
                            self.callback(False)
